...
if [ -n "$OUTPUT_INCLUDE" ] && [ "$ENTITIES_JSON" != "[]" ]; then
...

  {
    ...
    echo "# Rewritten by every network scan — edits are lost. Exclude players by long-pressing"
    echo "# their tile in the Settings view, then reload Groups + Automations."
