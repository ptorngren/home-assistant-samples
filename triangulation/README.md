...
`sensor.bt_device_names` maps each MAC address to the device name advertised with it, and the dashboard reads it purely for display. Entries with an empty or placeholder name are dropped when the map is rebuilt, but a real name is never evicted. Because BLE devices rotate their MAC address — typically every 15 minutes or so — a single chatty device mints new permanent entries several times an hour, and the map survives restarts through HA's restore-state store. It therefore grows for the life of the installation. One install measured 836 entries for only 126 distinct names — about 31 KB — with a single device accounting for 563 of them.
